...
class ComponentStatus:
    ...
    def __init__(self, branch:str=None, run:str=None, comp_only:bool=False):
        ...
        self.ts_component_summary = self.get_test_suites_data(data, comp_only)
        if not comp_only:
    ...
    def get_test_cases_data(self, data):
        ...
        try:
            ...
            df2 = tests_df[tests_df[f'{TESTCASE_PREFIX}status'].isin(status) | (tests_df[f'{TESTCASE_PREFIX}status'].isna())]
    ...
    def get_test_suites_data(self, data, comp_only:bool=False):
        component_summary = pd.DataFrame()
        components_list_df = pd.DataFrame()
        ...
        try:
            ...
            if comp_only:
                tests_df = pd.json_normalize(data, meta=['component', 'status'])

                df1 = pd.DataFrame(tests_df, columns=['component', 'name'])

                components_list_df = df1.copy() if components_list_df.empty else pd.concat([components_list_df, df1])

                # count tests results per component
                test_cases_df = pd.DataFrame(tests_df, columns=['component', 'status'])
                test_cases_count = test_cases_df.groupby(['component', 'status']).size().unstack().fillna(0)
                ...
                if not component_summary.empty:
                    components_list_df = components_list_df.drop_duplicates()
                    components_list_df.reset_index(inplace=True)
                    components_list_df = components_list_df.groupby(['component'])['name'].count().to_frame()
                    components_list_df.reset_index(inplace=True)
                    components_list_df = components_list_df[['component', 'name']]
                    components_list_df.set_index(['component'], inplace = True)
                    ...
                    component_summary = component_summary.astype({'pass_rate': 'float', 'tests_count': 'int32', 'passed': 'int32', 'failed': 'int32'
                                                                    , 'error': 'int32', 'skipped': 'int32', 'uniqe_suites': 'int32'})

                    component_summary = component_summary[['component', 'pass_rate', 'uniqe_suites', 'tests_count', 'passed', 'failed'
                                                            , 'error', 'skipped']]
                ...
                status = ['failed', 'blocked', 'error']
                # get test suites with status equal `failed` | `blocked` | `error` | `None`
                df2 = tests_df[(tests_df['status'].isin(status)) | (tests_df['status'].isna())]
                if not df2.empty:
