...
from pylib.twister.twisterlib.twister_platform import Platform, generate_platforms
            ...
            for dirpath, _, filenames in os.walk(root, topdown=True):
                ...
                try:
                    ...
                    for name in parsed_data.scenarios:
                        ...
                        logger.debug("Verifying platform_allow:")
                        logger.debug(f" Platform allow before: <{suite.platform_allow}>")
                        suite.platform_allow =  self.verify_platforms_existence(
                                suite.platform_allow,
                                f"platform_allow in {suite.name}")
                        logger.debug(f" Platform allow after: <{suite.platform_allow}>")
