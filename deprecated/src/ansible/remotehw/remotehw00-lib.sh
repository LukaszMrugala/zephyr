...
.remotehw-usb-common() {
	...
	$_ssh $1 "gunzip -c /64MB-FAT16.img.gz > /tmp/zephyr.disk"
	$_ssh $1 "/sbin/losetup -o 1048576 /dev/loop0 /tmp/zephyr.disk && mount /dev/loop0 /mnt/loop"
	$_ssh $1 "cd /mnt/loop && tar -xzf /zephyr-grub-boot-disk.tgz && sync"
}
...
.remotehw-usb-acrn() {
	...
	if [ -f "$2" ]; then
		echo " * deploying user-provided zephyr.bin ($2) to EFI/BOOT" 
